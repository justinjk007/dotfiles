 ...
 '(ledger-reports
   '(("bal cleared" "%(binary) [[ledger-mode-flags]] -f %(ledger-file) bal --cleared --real")
	...
	(idx 3))
     ...
     ("TFSA contrib bal" "%(binary) -S -T -f %(ledger-file) bal ^TFSAContribution ^CRA_TFSA_CALC_CONTRIBUTION")
     ("TFSA contrib reg" "%(binary) -S -T -f %(ledger-file) reg ^TFSAContribution ^CRA_TFSA_CALC_CONTRIBUTION")))
